...
const updateCache = async (config, sourcePath) => {
  ...
  if (customScripts && customScripts.length > 0) {
    log(3, '[cache] Using local custom scripts, skipping CDN fetches.');
    cache.sources = customScripts
      .map(s => readFileSync(join(__dirname, '..', s), 'utf8'))
      .join(';\n');
    extractVersion();
  ...
  for (const script of customScripts) {
    allFetchPromises.push(async () => fetchScript(script, proxyAgent));
  }
