...
const fetchScript = async (script, proxyAgent) => {
  try {
    ...
    const response = await fetch(`${url}.js`, requestOptions);
...
const updateCache = async (config, sourcePath) => {
  ...
  // Add core, modules, and indicators
  for (const script of allScripts) {
    allFetchPromises.push(async () => {
      const fullScript = `${config.cdnURL || cache.cdnURL}${script}`;
      const text = await fetchScript(fullScript, proxyAgent);

      // If fetched correctly, set it
      if (typeof text === 'string') {
        let moduleName = script.replace(
  ...
  for (const script of customScripts || []) {
    allFetchPromises.push(async () => {
      const text = await fetchScript(script, proxyAgent);
      return text;
    });
  }
  ...
  for (const fetchPromise of allFetchPromises) {
    ...
    cache.sources += text + ';\n';
    // Delay 3000ms to avoid rate limiting
